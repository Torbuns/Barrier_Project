...
from postprocessing import open_or_not


@smart_inference_mode()
def run(
        weights=ROOT / 'resource/5n6_mentor.pt',  # model path or triton URL
        source=ROOT / 'vidyvo.mp4',  # file/dir/URL/glob/screen/0(webcam)
        ...
        for i, det in enumerate(pred):  # per image
            ...
            if len(det):
                ...
                for *xyxy, conf, cls in reversed(det):
                    data.append([int(xyxy[0].item()), int(xyxy[1].item()), int(xyxy[2].item()), int(xyxy[3].item()),
                                 int(cls.item())])
        ...
        for i in cars.index:
            ...
            for j in licenses.index:
                ...
                if xmin_car < xmin_license and ymin_car < ymin_license and xmax_car > xmax_license and ymax_car > ymax_license:
                    ...
                    img_path = 'car_plate_number.jpg'
                    res_license = reader.ocr(img_path, cls=True)
                    open_or_not(res_license, reader, img_path)


def parse_opt():
    parser = argparse.ArgumentParser()
    parser.add_argument('--weights', nargs='+', type=str, default=ROOT / 'resource/5n6_mentor.pt',
